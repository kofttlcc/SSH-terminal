...
public class AIService {
    ...
    private func buildSystemPrompt(
        ...
        2. 【最高鐵律】作業系統與發行版一致性：執行具體指令前必須明確確認系統發行版。嚴禁跨發行版亂用包管理器（Debian/Ubuntu 嚴格使用 apt/dpkg；RHEL/CentOS/Rocky 嚴格使用 dnf/yum/rpm；Alpine 嚴格使用 apk；Arch 嚴格使用 pacman；macOS 嚴格使用 brew/launchctl）。
        ...
        5. Shell 語法嚴格合規：輸出的所有 Shell 指令必須是語法正確的 Bash/Linux 終端指令，嚴禁包含無效續行符或未閉合引號。
        6. 簡潔有力：除非使用者要求詳細教學，否則請保持回答精簡、直擊核心。
        """
